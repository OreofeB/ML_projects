...
CNNmodel.add(Conv2D(filters=16, kernel_size=(3,3), padding='valid', 
                    activation='relu', input_shape=(matrix_size, matrix_size, 1)))
CNNmodel.add(MaxPooling2D(2,2))
CNNmodel.add(Conv2D(256, 3, activation='relu'))
CNNmodel.add(MaxPooling2D(2,2))
CNNmodel.add(Flatten())
CNNmodel.add(Dropout(0.2, seed=123))
...
CNNmodel.compile(optimizer=Adam(), loss="binary_crossentropy", metrics=["binary_accuracy"])
CNNmodel.summary()
...
callbacks = [
    ...
    ModelCheckpoint('cnn_model.h5', verbose=1, save_best_only=True, save_weights_only=False)
]

# Step 4: Train the model using the cleaned and preprocessed data
CNNmodel.fit(X_train, y_train, validation_data=(X_valid,y_valid), batch_size=50, epochs=100, callbacks=callbacks)
